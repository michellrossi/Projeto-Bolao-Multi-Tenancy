    ...
    const checkRedirect = async () => {
      try {
        ...
        if (result?.user) {
          ...
          navigate(currentLeagueId ? '/app/palpites' : '/app/ligas');
        }
      } catch (err: any) {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (result.user) {
        ...
        if (accessCode.trim()) await validateAndApplyCode(u.uid);
        navigate(currentLeagueId ? '/app/palpites' : '/app/ligas');
      }
    } catch (err: any) {
            ...
            <AnimatePresence mode="wait">
              {isRegistering && (
                <div className="relative">
                  ...
                  <input type="text" placeholder="Seu nome" value={displayName} onChange={(e) => setDisplayName(e.target.value)} required={isRegistering} className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 pl-12 pr-4 text-sm focus:outline-none focus:border-primary transition-all text-white" />
                </div>
              )}
            </AnimatePresence>
            <div className="relative">
