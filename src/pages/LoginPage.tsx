...
export function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (user && !loading && !isResettingPassword) {
      console.log("User detected, navigating to palpites");
      navigate('/app/palpites');
    }
  }, [user, navigate, loading, isResettingPassword]);
  ...
  if (user && !isResettingPassword) return <Navigate to="/app/palpites" replace />;
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) throw error;
      setError('✓ Senha alterada com sucesso! Redirecionando...');
      setTimeout(() => {
    ...
    <div className="relative min-h-screen w-full flex items-center justify-center overflow-hidden font-sans bg-dark">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="relative z-10 w-full max-w-md px-6 py-12">
        <div className="glass-dark p-8 md:p-10 rounded-[3rem] shadow-2xl flex flex-col items-center border border-white/5">
          ...
          {isResettingPassword ? (
            <form onSubmit={handleResetPassword} className="w-full space-y-4 mb-8">
              <p className="text-white/60 text-xs text-center mb-4">Digite sua nova senha de acesso abaixo.</p>
              ...
              <button disabled={loading} type="submit" className="w-full bg-primary text-dark font-black py-4 rounded-2xl flex items-center justify-center gap-2 hover:bg-white transition-all uppercase tracking-widest text-xs disabled:opacity-50">
                {loading ? <Loader2 className="animate-spin" size={18} /> : <>Salvar Nova Senha <ArrowRight size={16} /></>}
              </button>
            </form>
          ) : (
            <form onSubmit={handleEmailAuth} className="w-full space-y-4 mb-8">
              ...
              <div className="relative">
                ...
                <input type="email" placeholder="E-mail" value={email} onChange={(e) => setEmail(e.target.value)} required className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 pl-12 pr-4 text-sm focus:outline-none focus:border-primary transition-all text-white" />
              </div>
              <div className="relative">
                <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-white/20" size={18} />
          ...
          {!isResettingPassword && (
            <>
              <div className="w-full space-y-4">
                <div className="flex items-center gap-4"><div className="h-[1px] flex-1 bg-white/5" /><span className="text-white/20 text-[10px] font-black uppercase tracking-widest">ou use o Google</span><div className="h-[1px] flex-1 bg-white/5" /></div>
                <button type="button" onClick={handleGoogleLogin} className="w-full bg-white text-dark font-black py-4 rounded-2xl flex items-center justify-center gap-3 hover:bg-primary transition-all uppercase tracking-widest text-[10px]">
                  <svg className="w-5 h-5" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" /><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" /><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81.38z" /><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.66l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" /></svg>
                  Login com Google
                </button>
              </div>
              <button onClick={() => setIsRegistering(!isRegistering)} className="mt-8 text-white/40 hover:text-primary transition-colors text-xs font-bold">{isRegistering ? 'Já tem uma conta? Entre aqui' : 'Não tem conta? Cadastre-se agora'}</button>
            </>
          )}
        </div>
      </motion.div>
